...
function generateInputFiles(asset_list){
    var writeFile = "";
    asset_name_tag = '<asset>';
    asset_name_tag_end = '</asset>';
    var validKeyList = ['asset_name','availability','confidentiality','data_source','frequency','integrity','threat_list'];
    for (var i = 0;i<asset_list.length;i++){
        writeFile += asset_name_tag + '\n';
        for (var key in asset_list[i]){
            if (!validKeyList.includes(key)) continue;
            if(key=='threat_list'){
                writeFile += '<threat_list>\n';
                for (var j=0;j<asset_list[i]['threat_list'].length;j++){
                    for (var keyThreat in asset_list[i]['threat_list'][j]){
                        writeFile += keyThreat + ":" +asset_list[i]['threat_list'][j][keyThreat]+",";
                    }
                    writeFile += '\n';
                }
                writeFile += '</threat_list>\n';
                 ...
                 writeFile += key + ":"+asset_list[i][key]+"\n";
            }
        }

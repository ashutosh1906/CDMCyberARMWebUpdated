...
class Threat(object):
    ...
    def printProperties(self):
        CyberARMPowerPlant.write_output_file.write("\nID : %s Name : %s\n" % (self.primary_key,self.threat_name))
        CyberARMPowerPlant.write_output_file.write("Threat Impact %s\n" % (self.threat_impact_asset))
        CyberARMPowerPlant.write_output_file.write("Threat Action ------->\n")
        for i in range(len(self.asset_threat_action)):
            CyberARMPowerPlant.write_output_file.write("                          Threat Action ID : %s Prob : %s" % (self.asset_threat_action[i],self.asset_threat_action_prob[i]))
        CyberARMPowerPlant.write_output_file.write("                          %s" % (self.asset_threat_action))
        CyberARMPowerPlant.write_output_file.write("                          %s" % (self.asset_threat_action_prob))
...
INSERT INTO `cyber_defense_matrix_norm` VALUES (3,1,1,1,'1.1',NULL),(3,1,2,1,'1.2',NULL),(3,1,3,1,'1.3',NULL),(3,1,4,1,'1.4','This Security Control helps to identify the authorized device. Therefore, it prevents the use of unauthorized devices e.g. Surveillance Camera at the Reconnaissance phase.'),(3,2,5,3,'1.5','Any device wanting to connect with the network should be authenticated to limit and control the connection of the devices with the network. This protects the network from the unauthorized devices that may deliver the malware.'),(3,2,6,7,'1.5','This security control can prevent the unauthorized devices from accessing the confidential information. Thus, it offers Protect at the Maintain phase.'),(3,2,7,3,'1.6','This security control validates and authenticates a device using Client Certificate which protects the private network from the malware or worm delivered by the unauthorized devices. '),(3,2,8,7,'1.6','Authentication using Client Certificate can protect the confidential or sensitive information from the unauthorized devices.'),(4,1,9,5,'2.1','This security control helps to identify the authorized software and therefore, it will prevent the Installation of the malicious software as it won\'t be identified by the security control.'),(4,2,10,5,'2.2','Any software that is not the member of whitelist, will be prevented from the installation. Therefore, this security control protects at Installation phase.'),(4,2,11,3,'2.4','If the application runs in Virtual machine, because of the isolated environment the attackers\' will not be able to deliver the attack. Besides, if a host is compromised, it will be confined to that environment, which ensures the prevention of spread of the malicious software.'),(4,1,12,4,'3.1','This security control stops the attacker in the Exploitation phase by maintaining the secure configuration while installing an application based on the recent and discovered vulnerabilities. This security control actually identifies the required secure configuration for applications.'),(4,1,13,4,'3.2','This security control maintains(identifies) the strict configuration which will be maintained by all the applications and OS used by the organization. This prevents the exploitation of the vulnerabilities.'),(4,2,14,7,'3.3','Protect the secure image from unauthenticated change. Therefore, without the authentication, installed malware or any other application will not be able to change the configuration.'),(4,2,15,1,'3.4','Secure channel provides encryption. Thus protects the system by interrupting the Reconnaissance of the attackers\' '),(5,3,16,7,'3.5','This security control can detect the changes in the files. It will be able to generate alarm if the confidential files are changed and thus, the installed malware or unauthorized changes can be prevented at its Maintain phase.'),(4,3,17,7,'3.6','It detects the unauthorized changes in the secure configuration of the applications. If a malware changes the configuration to maintain its activity, this security control will be able to detect the suspicious activity.'),(4,2,18,4,'3.7','This security control redeploy the configuration at regular interval. Therefore, it can remove the existing vulnerabilities from the system which may have been created due to authenticated or unauthenticated changes in the system.'),(4,1,19,4,'4.1','This security control identifies the existing vulnerabilities for risk assessment. This security control prevents the attacker at the exploitation phase by removing the vulnerabilities.'),(4,1,20,1,'4.2','This security control attempts to identify who is scanning;whether it\'s authenticated scanning or not. This helps to prevent the Reconnaissance of the attacker for exploring the vulnerabilities.'),(4,2,21,1,'4.3','It protects the system from unauthenticated vulnerability scanning.'),(4,1,22,4,'4.4','Explore the recent vulnerabilities that will help to update the risk management strategy.'),(4,2,23,4,'4.5','Patch removes the vulnerabilities and thus, stops the attacker at the exploitation phase.'),(4,1,24,1,'4.6','This security control attempts to identify who is scanning;whether it\'s authenticated scanning or not. This helps to prevent the Reconnaissance of the attacker for exploring the vulnerabilities'),(4,1,25,4,'4.7','From the back-to-back vulnerabilities report, this security control assesses the risk.'),(4,1,26,4,'4.8','This security control prioritizes risk based on its exploitability and potential impact. Thus it protects the system by removing the vulnerabilities. '),(1,2,27,7,'5.1','This security control minimizes the administrative privilege of a user so that the user (people) if compromised won\'t be able to maintain its activities with the administrative privilege of that compromised user.'),(1,1,28,7,'5.2','This security control identifies the administrative account that will have the administrative privileges for performing the critical activities. Though, the attacker may be able to install the malware and also will be successful in set up the communication with C2 server., he/she will not be able to do activities on objects that require the administrative privilege.'),(3,2,29,7,'5.3','Default passwords of all the devices should be changed to ensure more protective safeguards of all the objects on those devices. This security controls stops the attackers in the Maintain phase as the attacker will perform activities on objects immediately if it can\'t be prevented in this phase.'),(1,3,30,5,'5.4','It gives alarm when new account is added, removed or deleted. This security control works in the Installation phase as it detects the attack installation phase which is introduction of an account accessible by the attacker to maintain its activities on objects.'),(1,3,31,1,'5.5','This security control logs the unsuccessful login to detect the suspicious behavior which may be happening due to the Reconnaissance by the attackers (brute force attack) to know the actual credentials.'),(1,2,32,7,'5.6','Instead of knowing the credentials the attacker will never be able to reach at Maintain phase as it requires another factor such as biometrics, smart card etc. for authentication. '),(1,2,33,1,'5.7','Reconnaissance (or Brute Force) by the attacker will never be successful as it is less likely that he will be able to interpret the password of the account.'),(1,2,34,7,'5.8','Sudo in linux is a tool that has implemented such security control which doesn\'t let the logged in account do the activities on critical files or applications without knowing the password. Therefore, if the attacker gets the access of that account, still he requires the credentials.'),(3,2,35,3,'5.9','As the machine dedicated for the administrative tasks is isolated from the network or internet access. This will stop the attacker by preventing its delivery of weapons.'),(3,3,36,7,'6.1','Logging the activities using two synchronized time sources which ensures the consistent time in all logs, helps to detect the unusual activities of the attackers\'. So, this security control detects the attackers by analyzing its activities on objects which is actually the Maintain phase for the attacker.'),(3,3,37,7,'6.2','Audit log should be done in valid format.'),(4,1,38,7,'6.4','Identify the anomalous behavior by analyzing the logs.'),(2,3,39,3,'6.5','Log all traffic arrived at network devices, so the unusual traffic can be detected due to Delivery of suspicious contents.'),(2,3,40,1,'6.5','This security control can detect the suspicious behavior of the attacker caused by the reconnaissance of the attacker. For example, in Cross Fire attack, the attacker sends the traffic to the decoy or public servers from the bots to detect the load traffic of the link.'),(2,3,41,7,'6.6','This security control aggregates logs to devise a profile of usual activities so that the detection due to unusual activities will generate less false positives. '),(4,2,42,4,'7.1','Fully supported and updated web browsers or email clients will remove the recent or previously explored vulnerabilities and hence, it protects from the attackers in the installation phase.'),(4,1,43,5,'7.2','This security controls blocks all the applications which are not in the whitelist and also the application that has come from a unidentified domain. Whitelist identifies the application that are necessary for the activiities in the organization.'),(4,2,44,5,'7.3','Block the unnecessary scripts. It protects the system from installation of the malicious software.'),(2,3,45,3,'7.4','This security control detects the delivery of the malicious contents by URL from the compromised system. This security control logs the URL requests to detect the suspicious activity.'),(4,2,46,3,'7.5','There will two separated browsers where one will only be used in some particular safe web sites and the other will be used for visiting all the other untrusted web sites.'),(2,2,47,3,'7.6',''),(2,2,48,3,'7.7',''),(2,4,49,3,'7.8','Block (Respond) the malicious email attachments at its delivery phase.'),(3,3,50,4,'8.1','Monitor the behavior of the detected malware collected from all the devices with countermeasure functionality.'),(4,3,51,5,'8.2','It detects the suspicious malware activities during the Installation.'),(3,2,52,3,'8.3','It protects by controlling the use of external devices so that the attacker will not be able to deliver the malicious contents.'),(4,2,53,5,'8.4','It protects by stopping the applications that may have the exploitation features. '),(2,4,54,3,'8.5','Filter out the malicious contents before its arrival at the endpoints.'),(2,3,55,6,'8.6','Logging of DNS query will help to detect whether there is any attempt to connect with the C2 server.'),(4,2,56,7,'9.1','This stops the execution of any applications or services which are not required for business.'),(2,4,57,3,'9.2','This security control filters out all the traffic if these are not explicitly allowed.'),(4,3,58,7,'9.3','It detects if applications are running in unusual ports.'),(3,2,59,3,'9.4',''),(3,2,60,3,'9.5','This security control separates the critical services from other services so that the attacker will not be able to deliver attack to interrupt the critical services by exploiting the other services hosted in the same device or physical machine.'),(4,4,61,3,'9.6','This security control blocks any traffic requesting unauthorized services from the critical servers. It blocks (Respond) the service request (Application) at the Delivery kill chain phase.'),(5,5,62,7,'10.1','The attacker may alter or remove the data to interrupt the critical services to maintain its attack but the Backup of such critical information will restore the services.'),(5,5,63,7,'10.2','It backups the data and also test whether the backup works fine. This security control helps to restore the system. '),(5,5,64,7,'10.3','This security control stores the backup and also protects it with the encryption in order to restore the system. or critical services.'),(5,5,65,7,'10.4','If the backup destination is not accessible by the attacker e.g. CryptoLocker, it will not be able to modify the data. Therefore, the backup data can be used to recover its normal activities.'),(3,2,66,4,'11.1','By maintaining the standard configuration it protects the system by removing the exploitable vulnerabilities.'),(2,3,67,3,'11.2','The change in configuration rules for traffic flow should be logged with the specific business reason, person and duration to detect the unusual behavior.'),(3,3,68,7,'11.3','All changes in the configuration that deviates from the standard configuration should be logged to detect the unusual change in the system.'),(2,2,69,7,'11.4','This security control ensures the protection of the session.'),(3,2,70,4,'11.5','It provides the security related updates for the network devices.'),(2,4,71,3,'12.1',''),(2,4,72,6,'12.1',''),(2,3,73,3,'12.2','In DMZ networks, there will be a monitoring system to monitor the traffic for detecting malicious or suspicious activities. Now, this security control operates in the delivery phase as the adversary may use the DMZ network for spreading or sending the attack to the other critical assets of the organization.'),(2,3,74,7,'12.3','This security control performs the behavioral analysis on the traffic flow to detect the attack. For example, if the attacker exfiltrates encrypted sensitive data, the high entropy in the network traffic and sometimes the number of flows to some specific IP address will draw attention as unusual activity. '),(2,4,75,3,'12.4','It prevents the bad traffic, so it interrupts the delivery of weapons by the adversary.'),(2,4,76,6,'12.4','It can respond on the bad traffiic that may be sent by the adversary C2 server to perform its objective on victim\'s machine.'),(2,4,77,6,'12.5','By decrypting and analyzing the network traffic, it will be able to prevent the communication request to the bad sites.'),(2,3,78,7,'12.5','It can decrypt and analyze the network traffic to inspect whether an exfiltration is happening. '),(1,2,79,7,'12.6','This security control will help to protect the system despite of the compromise of user credentials. It will generate one time token or password that will be sent to the legitimate user and the user will have to provide this password to login into the system.'),(1,2,80,7,'12.6','This security control will help to protect the system despite of the compromise of user credentials. It will generate one time token or password that will be sent to the legitimate user and the user will have to provide this password to login into the system. Therefore, this security control will interrupt the access to the system so that it will prevent the adversary in the Maintain kill chain phase.'),(3,2,81,4,'12.7','There must be policy which will define the access privilege for the remote devices that are trying to connect with internal network.'),(2,3,82,6,'12.8','This security control scans the network to check if there is any unauthorized connection to the internet. So, this security will be able to detect the C2 communication.'),(2,3,83,7,'12.9','This security control can analyze the network flow which will be able to detect the data exfiltration or other anomalous activity.'),(2,3,84,7,'12.10','Firewall session tracking will be able to identify the unusual long time TCP session. So from these TCP sessions the channel used for data exfiltration can be detected. '),(5,1,85,7,'13.1','Identify the sensitive data so that it can be protected by applying the encryption. The main objective of this security control is the identification at data level. This will prevent the loss of confidentiality of the exfiltrated data. Therefore, it works in the maintain phase.'),(5,2,86,7,'13.2','This security control encrypts the data in the devices and thus, protect the exfiltrated data. Therefore, this security control protects the data at the maintain phase.'),(5,3,87,7,'13.3','This security control detects the data exfiltration by deploying an automated tool	on network perimeters that monitors for sensitive information.	'),(5,1,88,1,'13.4','It identifies the sensitive information, so that the confidential information will not be leaked. Hence, it prevents the attackers in the reconnaissance phase by not leaking sensitive data.'),(5,2,89,7,'13.5','No data can be written in unauthorized USB hard drives. Moreover, this security control will only authorize USB devices with business need. Besides, this security control will encrypt all the transferred data on such authorized USB devices. Therefore, it prevents the data exfiltration when the attack is in the maintain kill chain phase. This security control provides protection for data.'),(2,3,90,7,'13.6','This security control monitors the network traffic to detect the anomaly. It works against the data exfiltration by the malicious software. Therefore, it prevents the attack in the maintain kill chain phase. '),(2,3,91,6,'13.7','This security control monitors the network traffic to detect the unauthorized use of encryption that may be used by the C2 channel. Therefore, this security control prevents the use of command and control channel by the attackers.'),(2,2,92,6,'13.8','This security control blocks the access to known file transfer and email exfiltration websites (black listed network url) which protects the asset from C2 channel. Therefore, this security controls protects the asset in the network level at the C2 kill chain phase of the attack.'),(5,2,93,7,'13.9','This security control provides ACL (Access Control List) for data while copying the data. Thus, any unauthorized entity will not be able to copy the data. Hence, this security control provides the protection at the Maintain kill chain phase for the data. '),(3,2,94,3,'14.1','Based on the stored information, this security control labels the device to segment the networks. People or other devices with authorization will only be able to access the device with sensitive information. The adversary will not able to deliver the attack weapons as it will not be able to access the device with sensitive information. Therefore, this security control protects the device at the Delivery kill-chain-phase.'),(5,2,95,1,'14.2','This security control encrypts sensitive information while communicating. Hence, it protects the sensitive information by encrypting the contents of the low trust network traffic which prevents the attacker at the Reconnaissance kill-chain-phase.'),(2,2,96,3,'14.3','This security controls stops the attack propagation by segmenting the network using Private Virtual Local Area Network (VLANs) in the switches which limits the ability of communication of one device with the other devices of the network. Therefore, this security control works on the network to protect the devices of the subnet against the attack propagation from the infected devices.'),(1,2,97,7,'14.4','This security control enforces access control list for individual (People) so that the adversary will not be able to exfiltrate the sensitive information. Hence, this security control works in the Maintain kill-chain-phase to protect the sensitive information by limiting the access for people.  '),(1,2,98,7,'14.5','This security control introduces authentication mechanism (Protect) of People for the sensitive information so that it will not be exfiltrated (Maintain).'),(1,2,99,7,'14.6','This security control enforces the authentication system on individual (People) to protect the sensitive information. Therefore, it works in the Maintain kill chain phase.'),(1,3,100,7,'14.6','This security control will keep the log of the access of sensitive information by individuals which will always keep the record of the unsuccessful access attempts. Hence, it will detect the attackers by the number of unsuccessful events.'),(3,2,101,3,'14.7','This security control will protect the system or devices storing the archived data sets by disconnecting it from the network. Therefore, it will prevent the attacker from delivering the attack weapons.'),(2,2,102,4,'15.1','Only the devices with the authorized configuration and security profile with the reason of connection will be allowed. Therefore, this security control will protect the organization from the attacker in the exploitation kill chain phase.'),(2,4,103,4,'15.2','This security control detects the unauthorized wireless access points by comparing the connected device at a particular access point with the list of identified devices. Any wireless access point connecting the unauthorized device will be deactivated. Therefore, this security control responds against the attacker in the exploitation phase.'),(2,3,104,5,'15.3','This security control will detect the rouge wireless devices by analyzing the wireless network traffic. It works in the exploitation kill chain phase as it can detect the attack attempt or successful compromise.'),(3,1,105,4,'15.4','This security control identifies the wireless access of devices with the proper business requirements. Thus, it prevents the unauthorized devices in the exploitation kill chain phase.'),(5,2,106,1,'15.5','Wireless traffic are encrypted using Advanced Encryption Standard encryption with Wi-Fi Protected Access 2 (WPA2) protection.'),(3,2,107,4,'15.6','This security control enforces authentication protocol such as Extensible Authentication Protocol-Transport Layer Security (EAP/TLS) with mutual authentication to protect the wireless network. So, this security control can prevent the attacker at the exploitation kill chain phase.'),(2,2,108,7,'15.7','Disable peer to peer network network communication so that all the file or data transfer must be transferred via the server. It protects the assets or the organizations from the malicious communication in the Maintain kill chain phase.'),(3,2,109,4,'15.8','It protects in the exploitation kill chain phase by disabling the peripheral access of devices (such as bluetooth) which have no business requirements.'),(3,2,110,4,'15.9','BYOD systems and other untrusted devices will be able to access the enterprise network through virtual local area network. Therefore, the untrusted devices are prevented in the exploitation kill chain phase.');
...
INSERT INTO `kill_chain_phase` VALUES ('Reconnaissance',1),('Weaponization',2),('Delivery',3),('Exploitation',4),('Installation',5),('C2',6),('Maintain',7);
...
-- Dump completed on 2017-09-22 21:02:42

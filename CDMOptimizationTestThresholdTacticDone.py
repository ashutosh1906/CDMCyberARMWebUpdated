...
def SMT_Environment(security_control_list,selected_security_controls,threat_action_name_list,threat_action_list,
    ...
    # print "Threat Actions List %s" % (threat_action_id_list_for_all_assets)
    ...
    ############################################################ Declare SMT Solver #####################################################
    # cyberARMTactic = Then(Repeat(OrElse(Tactic('split-clause'),
    #                                  Tactic('skip'))),
    ...
    for asset_index in range(len(threat_id_for_all_assets)):
        ...
        for threat_id in threat_id_for_all_assets[asset_index]:
            ...
            if len(smt_Threat_Threat_Action_Defense_Success[asset_index][threat_id_index]) > 0:
                ...
                cyberARMGoal.add(smt_Threat[asset_index][threat_id_index]==threat_list[threat_id].maximum_risk[asset_index])
            threat_id_index += 1
    ...
    #                                  Tactic('skip'))),
    #                    Tactic('solve-eqs')).solver()
    # resultSimplified = Then('simplify', 'solve-eqs')(cyberARMTactic)
    # print "Length of Goals %s" % (len(resultSimplified))
    # cyberARM = Solver()
    # cyberARM.add(resultSimplified[0])
    ...
    # ####################################### Check the Remaining of a Threat Action ##########################################################
    # for i in range(len(threat_action_id_list_for_all_assets)):
    #     print "<---Asset Index %s---->" % (i)
    #     print "Threat Action Success %s" % (recommended_CDM[smt_Threat_Action_Success[i][0]])
    #     for j in range(len(threat_id_for_all_assets[i])):
    #         print "        <--------------Threat Action Index %s: Defense Effectiveness %s----------------------->" % (j,recommended_CDM[smt_Threat[i][j]])
    ...
    for asset_index in range(len(asset_list_for_smt)):
        ...
        for sec_control in selected_security_controls[asset_index]:
            ...
            # print "Security Control id: %s Threat Action List : %s" % (sec_control,security_control_list[sec_control].global_asset_threat_action_list[asset_index])
            # print "Threat Action Effectiveness %s" % (security_control_list[sec_control].threat_action_effectiveness)
    ...
    for asset_index in range(len(threat_id_for_all_assets)):
        ...
        for threat_id in threat_id_for_all_assets[asset_index]:
            ...
            threat_action_index = 0
            for threat_action in threat_list[threat_id].global_asset_threat_action[asset_index]:
                print "************** Threat Action (%s,%s,%s) ********************" % (threat_action,threat_action_id_to_position_roll[asset_index][threat_action],
